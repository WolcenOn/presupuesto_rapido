...
func main() {
	...
	if cfg.MailWorkerEnabled { mail.Worker{DB: db, Config: mail.Config{Host: cfg.SMTPHost, Port: cfg.SMTPPort, Username: cfg.SMTPUsername, Password: cfg.SMTPPassword, FromEmail: cfg.SMTPFromEmail, FromName: cfg.SMTPFromName}}.Start(ctx) }
	...
	ownerOnly := chain(authn, httpx.RequireRole("owner"))
	managerOnly := chain(authn, httpx.RequireRole("owner", "boss"))
	...
	mux.Handle("GET /api/documents", authn(http.HandlerFunc(h.TenantListDocuments)))
	mux.Handle("POST /api/documents", authn(http.HandlerFunc(h.TenantCreateDocument)))
	mux.Handle("GET /api/documents/{id}", authn(http.HandlerFunc(h.TenantGetDocument)))
	mux.Handle("POST /api/documents/{id}/send-to-boss", authn(http.HandlerFunc(h.TenantQueueDocumentForBoss)))
	...
	go func() { log.Printf("api listening on :%s (%s)", cfg.Port, cfg.Env); if err := server.ListenAndServe(); err != nil && !errors.Is(err, http.ErrServerClosed) { log.Fatalf("server error: %v", err) } }()
	<-ctx.Done()
