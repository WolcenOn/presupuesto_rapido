...
func (h Handler) UpdateCompany(w http.ResponseWriter, r *http.Request) {
	...
	_, err := h.DB.Exec(r.Context(), `update companies set name=$1, tax_id=$2, email=$3, phone=$4, address=$5, city=$6, postal=$7, province=$8, country=$9, updated_at=now() where id=$10`, input.Name, input.TaxID, input.Email, input.Phone, input.Address, input.City, input.Postal, input.Province, valueOr(input.Country, "ES"), companyID)
