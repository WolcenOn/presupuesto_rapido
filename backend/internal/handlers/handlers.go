...
func (h Handler) ListPrices(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.DB.Query(r.Context(), `select id, name, base_price, iva_rate, active, coalesce(updated_by::text, ''), updated_at from price_items where active = true order by name asc`)
	if err != nil {
...
func (h Handler) ListDocuments(w http.ResponseWriter, r *http.Request) {
	...
	query := `select id::text, ref, type, employee_id::text, client_name, client_cif, client_phone, client_address, coalesce(work_order, ''), coalesce(payment_method, ''), base_amount, iva_amount, total_amount, pdf_path, sent_to_boss_at, created_at, updated_at from documents`
	args := []any{}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&d.ID, &d.Ref, &d.Type, &d.EmployeeID, &d.ClientName, &d.ClientCIF, &d.ClientPhone, &d.ClientAddress, &d.WorkOrder, &d.PaymentMethod, &d.Base, &d.IVA, &d.Total, &d.PDFPath, &d.SentToBossAt, &d.CreatedAt, &d.UpdatedAt); err != nil {
...
func (h Handler) CreateDocument(w http.ResponseWriter, r *http.Request) {
	...
	err := h.DB.QueryRow(r.Context(), `
		...
		on conflict (employee_id, ref) do update set
			...
			updated_at = now()
		returning id::text`, input.Ref, input.Type, user.ID, input.ClientName, input.ClientCIF, input.ClientPhone, input.ClientAddress, input.WorkOrder, input.PaymentMethod, input.Base, input.IVA, input.Total, []byte(input.DocumentJSON)).Scan(&id)
	if err != nil {
